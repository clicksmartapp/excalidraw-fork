...
import { PixelEraserTrail } from "../eraser/pixelEraser";
...
class App extends React.Component<AppProps, AppState> {
  ...
  pixelEraserTrail = new PixelEraserTrail(this.animationFrameHandler, this);
  ...
  componentDidUpdate(prevProps: AppProps, prevState: AppState) {
    ...
    if (
      ...
      (prevState.theme !== this.state.theme ||
       prevState.currentEraserSize !== this.state.currentEraserSize)
    ) {
      // Update cursor with new theme or size
      setEraserCursor(this.interactiveCanvas, this.state.theme, this.state.currentEraserSize);
    }
    ...
    if (isEraserActive(prevState) && !isEraserActive(this.state)) {
      // End both eraser trails
      this.eraserTrail.endPath();
      this.pixelEraserTrail.endPath();
    }
  ...
  private handleEraser = (
    ...
    // Determine which eraser to use based on state
    const usePixelEraser = this.state.pixelEraserEnabled;

    let elementsToErase: string[] = [];

    if (usePixelEraser) {
      // For pixel eraser, we need to track both trails
      // The regular eraser trail is used for element detection
      // The pixel eraser trail is used for actual erasing

      // First, detect elements with the regular eraser
      elementsToErase = this.eraserTrail.addPointToPath(
        scenePointer.x,
        scenePointer.y,
        event.altKey,
      ) as string[];

      // Then add the point to the pixel eraser trail for pixel-based erasing
      this.pixelEraserTrail.addPointToPath(
        scenePointer.x,
        scenePointer.y
      );
    } else {
      // For regular eraser, just use the regular eraser trail
      // Eraser size will be read from app state when needed
      elementsToErase = this.eraserTrail.addPointToPath(
        scenePointer.x,
        scenePointer.y,
        event.altKey,
  ...
  private handleCanvasPointerDown = (
    ...
    if (this.state.activeTool.type === "eraser") {
      // Determine which eraser to use
      const usePixelEraser = this.state.pixelEraserEnabled;

      if (usePixelEraser) {
        // Use pixel eraser - make sure size is updated
        this.pixelEraserTrail.updateSize();
        this.pixelEraserTrail.startPath(
          pointerDownState.lastCoords.x,
          pointerDownState.lastCoords.y,
        );
      } else {
        // Use regular eraser
        this.eraserTrail.updateSize();
  ...
  private eraseElements = () => {
    // Check if we're using pixel eraser mode
    if (this.state.pixelEraserEnabled) {
      console.log("Using pixel eraser mode for erasing");
      
      // Get all elements that need to be processed
      const elementsToProcess = Array.from(this.elementsPendingErasure);
      ...
      // Get the pixel eraser trail for accurate erasing
      const pixelEraserTrail = this.pixelEraserTrail.getCurrentTrail();
      
      // Get all points from the pixel eraser trail
      const eraserPoints = pixelEraserTrail?.originalPoints?.map(
        p => pointFrom<GlobalPoint>(p[0], p[1])
      ) || [];
      
      const eraserSize = this.state.currentEraserSize || 10;
      
      console.log(`Using ${eraserPoints.length} eraser points for pixel erasing`);
      ...
      for (const elementId of elementsToProcess) {
        ...
        if (isFreeDrawElement(element)) {
          console.log(`Processing freedraw element ${element.id} for pixel erasing`);
          ...
          console.log(`After erasing: ${newPoints.length} points remain out of ${element.points.length}`);
          ...
          if (newPoints.length < 2) {
            ...
            modifiedElements.push(newElementWith(element, { isDeleted: true }));
          } else if (newPoints.length < element.points.length) {
            // Only update if we actually removed some points
            modifiedElements.push(newElementWith(element, { points: newPoints }));
          }
        } else if (isLinearElement(element)) {
          console.log(`Processing linear element ${element.id} for pixel erasing`);
          ...
          console.log(`Using traditional erasing for element type: ${element.type}`);
          modifiedElements.push(newElementWith(element, { isDeleted: true }));
        }
      }
      
      if (modifiedElements.length > 0) {
        console.log(`Modified ${modifiedElements.length} elements with pixel eraser`);
        
        // Create a map of modified elements for quick lookup
        const modifiedElementsMap = new Map(
          modifiedElements.map(element => [element.id, element])
        );
        
        // Update the scene with the modified elements
        const updatedElements = this.scene.getElementsIncludingDeleted().map(element => {
          const modifiedElement = modifiedElementsMap.get(element.id);
          return modifiedElement || element;
        });
      ...
      // Clear the pending erasure set
      this.elementsPendingErasure = new Set();
      return;
    }
    
    // Traditional full-stroke erasing
    console.log("Using traditional eraser mode");
    let didChange = false;
